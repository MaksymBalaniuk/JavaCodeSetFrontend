...
export class CodeBlockEditComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    if (this.dataLoadContextService.codeBlockContentAutoFill != '') {
      this.codeBlockContent = this.dataLoadContextService.codeBlockContentAutoFill;
    }
  }
  ...
  ngOnDestroy(): void {
    ...
    this.dataLoadContextService.codeBlockContentAutoFill = '';
  }
      ...
      .subscribe(tags => {
        tags.forEach(tag => {
          if (!this.tags.includes(tag.name)) {
              this.tagService.deleteTagFromCodeBlock(tag.id, codeBlock.id, token)
                .subscribe();
          }
        });
      });
    ...
    this.tags.forEach(tag => {
      if (!this.loadedTags.includes(tag)) {
        this.tagService.createTag({
          ...
          this.tagService.addTagToCodeBlock(tag.id, codeBlock.id, token)
            .subscribe();
        });
      }
    });
